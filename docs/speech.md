...
Three functions, and no platform conditionals anywhere in calling code:
...
|---|---|---|---|
| macOS, iOS | `AVSpeechSynthesizer` | Swift | the synthesizer must outlive the call — one file-scope `let` in Swift |
...
`day-part-battery` ([battery.md](battery.md)) proved a headless crate can contribute a platform
implementation. This crate is the case that motivated [daybridge](bridge.md) itself: **six
languages, one file, one declaration**, with no hand-written glue, no per-language directory of
shims, and no wire format to keep in agreement. The whole crate is `src/lib.rs` plus a manifest.

It is also the honest test of the design's limits. Speech is fire-and-forget, which is the only
